...
def install_rom(boot, rom, gapps, vendor, magisk):
    ...
    run_command(f"{fastboot} reboot recovery", "Reiniciando en modo recovery...", wait=False)
    run_command(f"{adb} -d sideload {partition}", f"Instalando el archivo {PAT}...")
    run_command(f"{adb} -d reboot recovery", "Reiniciando en modo recovery...", wait=False)
    run_command(f"{adb} -d sideload {rom}", f"Instalando la ROM desde {rom}...")
    run_command(f"{adb} -d reboot bootloader", "Reiniciar en modo bootloader...", wait=False)
